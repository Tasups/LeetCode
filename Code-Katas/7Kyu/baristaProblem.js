function barista(coffees) {
  if (coffees === 0) {
    ...
    const sortCoffees = coffees.sort((a, b) => a - b)
    console.log(sortCoffees)
    return sortCoffees
  }

  let total
  for (let i = 0; i < sortCoffees.length; i++){
    if (i == 0) {
      total += sortCoffees[i] * (sortCoffees.length - 1)
      total += 2 * (sortCoffees.length - 1)
    } else if (i == sortCoffees.length - 1) {
      total += (sortCoffees[i] * (sortCoffees.length - [i])) + 2
    } else {
      total += sortCoffees[i] * (sortCoffees.length - [i + 1])
    }
  }
}

barista([])//returns 0
barista([2, 10, 5, 3, 9])//returns  85
barista([4, 3, 2])//returns  22
barista([20, 5])//returns  32
barista([20, 5, 4, 3, 1, 5, 7, 8]),//returns 211
barista([5, 4, 3, 2, 1])//returns 55
