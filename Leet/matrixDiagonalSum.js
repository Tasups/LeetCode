...
function matDiagSum (mat) {
  ...
  while(count !== Math.floor(mat.length / 2) && count !== Math.ceil(mat.length / 2)) {
    // top left position
    console.log(mat[count][count]);
    total += mat[count][count];
    // top right position
    console.log(mat[count][mat[count].length - (count + 1)]);
    total += mat[count][mat[count].length - (count + 1)];
    // bottom left position
    console.log(mat[mat.length - count - 1][count]);
    total += mat[mat.length - count - 1][count];
    // bottom right position
    console.log(mat[mat.length - count - 1][mat[count].length - count - 1]);
    total += mat[mat.length - count - 1][mat[count].length - count - 1];
    count++;
  }
  
  if (mat.length % 2 !== 0) {
    console.log(mat[Math.floor(mat.length / 2)][Math.floor(mat[Math.floor(mat.length / 2)].length / 2)]);
